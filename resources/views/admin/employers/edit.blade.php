        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Edit Employer
                    </h5>
                </div>
                <div class="card-body">
                    <form class="row g-3" action="{{ route('admin.employers.update', $employer->id) }}" method="post"
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('package_id') is-invalid @enderror" name="package_id"
                                ...
                                @foreach ($packages as $key => $value)
                                    <option value="{{ $key }}" @if (old('package_id', $employer->package_id) == $key) selected @endif>
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                            @error('package_id')
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                value="{{ old('name', $employer->name) }}" name="name" required />
                            @error('name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control @error('earning_amount') is-invalid @enderror"
                                value="{{ old('earning_amount', $employer->earning_amount) }}" name="earning_amount"
                                required />
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control @error('min_amount_to_move') is-invalid @enderror"
                                value="{{ old('min_amount_to_move', $employer->min_amount_to_move) }}"
                                name="min_amount_to_move" required />
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('is_active') is-invalid @enderror" name="is_active" required>
                                ...
                                <option value="1" @if (old('is_active', $employer->is_active)) selected @endif>Active</option>
                                <option value="0" @if (!old('is_active', $employer->is_active)) selected @endif>Inactive</option>
                            </select>
                            @error('is_active')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
